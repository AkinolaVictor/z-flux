...
type AnimationOrder = "firstToLast" | "lastToFirst" | "random";
...
interface ZTextProps {
  ...
  extendAnimation?: Record<string, unknown>;
...
export default function Z_Text_ss({
  text,
  scrollingElement,
  progression = "char",
  animation,
  ...
  const useAnimation =
    animation_list[
      animation as keyof typeof animation_list
    ] ?? {};
  ...
  function initi_animation() {
    ...
    const ctx = gsap.context(() => {
      const scroller = scrollingElement
        ? document.querySelector(scrollingElement)
      ...
      if (grouped) {
        progressionData.animate.forEach((item: any) => {
          tl.to(
            item.char,
            {
  ...
  function updateScreenResize() {
    ...
    const update = () => setResizeTick((v) => v + 1);
